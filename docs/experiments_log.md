...
**Сколько всего классов на UGC test.** Формально в `categories` внутри
`test_coco.json` — 7 записей (living, bedroom, bathroom, kitchen, balcony,
wall, opening), это те же 7, что и на ResPlan/CubiCasa. Но если считать
честно, по факту участвующих в оценке классов — их **9**:
- 7 foreground-классов выше;
- **background** (класс 0, неявный, но участвует в TN/precision-recall);
- **room** — не формальная COCO-категория (её никто не предсказывает), а
  отдельный GT-бакет из `ignore_regions` (`raw_name="room"`): предсказание
  засчитывается верным, если это **living ИЛИ bedroom** в этом месте
  (см. `eval/compute_pixel_metrics.py`, `eval/compute_confusion_matrix.py`).
  Технически это не 8-й элемент в `categories`, а способ оценки, но по
  смыслу участвующих в метриках классов их именно 9.
...
| RF-DETR fullaug (thresh=0.1) | resplan | 0.866 | 0.912 | 0.908 | 0.952 |
| RF-DETR fullaug (thresh=0.1) | cubicasa | 0.498 | 0.734 | 0.714 | 0.833 |
| RF-DETR fullaug (thresh=0.1) | **всё** | **0.815** | **0.884** | 0.884 | 0.939 |

### RF-DETR fullaug — на порядок лучше base, и это НЕ утечка данных

Добавили сюда же **RF-DETR fullaug** (тот же чекпоинт, что и на UGC,
эпоха 8/100). В отличие от base, у него на этом домене calibration
нормальная (90% сырых боксов ≥0.02, 27% ≥0.1) — оставили стандартный
порог 0.1. Результат разительно лучше base RF-DETR: mIoU 0.815 против
0.016, сопоставимо с UNet (0.787) или даже чуть выше.

⚠️ Но: `data_v2/valid_files.txt` (val-сплит, на котором обучался fullaug)
пересекается с нашим `data/valid_files.txt` только на **806 из 4016**
картинок (20%) — то есть ~80% нашего val-сплита теоретически МОГЛИ быть
в train-данных fullaug (другой train/valid split, свой seed на
data_v2). Чтобы исключить утечку, пересчитали все три модели ТОЛЬКО на
честном пересечении (806 картинок, гарантированно held-out и для base,
и для fullaug):

| Модель | mIoU (per-class) | F1 (per-class) | class-agnostic IoU |
|---|---:|---:|---:|
| RF-DETR (base) | 0.016 | 0.036 | 0.529 |
| **RF-DETR fullaug** | **0.806** | **0.877** | 0.882 |
| UNet (сырой) | 0.789 | 0.867 | 0.877 |

Практически идентично полной выборке (0.815 vs 0.806) — **превосходство
fullaug над base подтверждено, это не артефакт утечки**. fullaug
действительно намного лучше откалиброван и обучен, чем base RF-DETR
(эпоха 8 против эпохи 4-5, плюс другой датасет с аугментациями), и на
этом собственном домене (в отличие от UGC, где на момент теста fullaug
был ХУЖЕ base) — превосходит его с большим отрывом.
